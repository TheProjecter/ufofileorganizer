...
public class FinderTableModel extends AbstractTableModel {
	...
	private String[] columnNames = {"Name","Size","Type","Location","Disk","Container"};
	private Vector<String[]> cells = new Vector<String[]>();
	private TreeNodeVector _TreeNodeVector;

	public FinderTableModel(TreeNodeVector aTreeNodeVector) {
	...
	private void update() {
		...
		for (int i = 0; i<tempChildrenVector.size();i++){
			...
			row_temp[3] = ((TreeNodeVector) _TreeNodeVector.get(i) ).getLocation();
                        row_temp[4] = ((TreeNodeVector) _TreeNodeVector.get(i) ).getParentGroupName();
                        row_temp[5] = ((TreeNodeVector) _TreeNodeVector.get(i) ).getParentContainerName();
			cells.add(row_temp);
		}
